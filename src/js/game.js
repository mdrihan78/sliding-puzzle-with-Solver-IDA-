// Game state and rendering logic for Sliding Puzzle

window.puzzleGame = (() => {
  const modes = { easy:{size:3}, normal:{size:4}, hard:{size:5} };
  ...
  function createGrid(gridSize){
    ...
    puzzleGame.updateProgressBar();
    if(window.timerLogic) window.timerLogic.resetTimer();
  }
  function renderGrid() {
    gridEl.innerHTML = '';
    for(let i=0;i<tiles.length;i++) {
  ...
  function getState() {
    return {size, tiles: tiles.slice(), emptyIndex, currentMode};
  }
  function setState(newTiles) {
    tiles = newTiles.slice();
    emptyIndex = tiles.indexOf(null);
    renderGrid();
  }
  function getGridSize() { return size; }
